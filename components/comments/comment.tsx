...
const Comment = ({
  ...
  const [childCommentList, setChildCommentList] = useState<CommentResponse[]>(
  ...
  const [likeCount, setLikeCount] = useState(
    commentResponse?.likes?.length || 0
  );
    ...
    const client = new Client({
      ...
      onConnect: () => {
        ...
        client.subscribe(`/topic/comments/${blog.blogId}`, (message) => {
          ...
          if (newComment.parentComment.commentId !== commentResponse.commentId)
            return;
        ...
        client.subscribe(
          `/topic/likes/${commentResponse.commentId}`,
          (message) => {
            const newLikeComment = JSON.parse(message.body);
            if (
              newLikeComment.comment.commentId === commentResponse.commentId
            ) {
        ...
        client.subscribe(
          `/topic/likes/dislike/${commentResponse.commentId}`,
          (message) => {
  ...
  const handleDeleteComment = async () => {
    ...
    if (deleteComment.status === 200) {
      setComments((prev) =>
        prev.filter((comment) => comment !== commentResponse)
      );
      refreshBlog();
    }
  };
  ...
  useEffect(() => {
    const fetchChildComment = async () => {
      if (
        ...
        return;
      const getChildrenComment = await sendRequest<
      ...
      <div className="flex justify-between">
        <div>
          <div className="flex gap-x-5 my-5">
            <div>
              <Avatar
                alt={commentResponse.user.fullname}
                src={`${storageUrl}/avatar/${commentResponse.user.avatar}`}
              />
            </div>
            <div>
              ...
              <span className="text-sm text-[#bde0fe]">
                {dateFormatter.format(Date.parse(commentResponse.createdAt))}
              </span>
            </div>
          </div>
          <div className="pl-3">{commentResponse.content}</div>
          <div className="flex gap-x-8">
            <div className="flex gap-x-3">
              <Tooltip title="Nhấn để thích bài viết này" placement="top" arrow>
                {/* <IconButton color={statusLike === false ? "default" : "primary"}> */}
                <IconButton
                  color={likeComment ? "error" : "default"}
            ...
            <div className="flex gap-x-3">
              <Tooltip
                ...
                {/* <IconButton color={statusLike === false ? "default" : "primary"}> */}
                <IconButton
                  color={childCommentList?.length > 0 ? "primary" : "default"}
                  onClick={() =>
                    toggleChildrenVisibility(commentResponse.commentId)
                  }>
